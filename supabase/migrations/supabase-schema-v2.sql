DROP VIEW IF EXISTS v_rujuk_rs CASCADE;
DROP VIEW IF EXISTS v_sick_list CASCADE;
...
DROP TRIGGER IF EXISTS trg_kunjungan_berobat_updated ON kunjungan_berobat;
DROP TRIGGER IF EXISTS trg_absensi_sakit_updated ON absensi_sakit;
DROP TRIGGER IF EXISTS trg_man_power_updated ON man_power;
DROP TRIGGER IF EXISTS trg_employees_updated ON employees;
DROP FUNCTION IF EXISTS update_updated_at();

DROP TABLE IF EXISTS kunjungan_berobat CASCADE;
DROP TABLE IF EXISTS absensi_sakit CASCADE;
DROP TABLE IF EXISTS man_power CASCADE;
DROP TABLE IF EXISTS employees CASCADE;

-- ═══════════════════════════════════════════════════════════
--  QSHE Department — Supabase Schema
--  PT. BAGONG DEKAKA MAKMUR
--  Versi: 1.1 (fixed: karyawan_lebih_3hari column in view)
-- ═══════════════════════════════════════════════════════════

-- ─────────────────────────────────────────────
--  TABEL 0: employees (Master Data Karyawan)
--  Di-sync otomatis dari Google Sheets via /api/sync-employees.
--  Satu baris = satu karyawan.
-- ─────────────────────────────────────────────
...
COMMENT ON COLUMN employees.employment_status IS 'Aktif / Withdrawn';
COMMENT ON COLUMN employees.site_name IS 'Nama site/lokasi kerja — digunakan untuk join dengan tabel lain';

-- Indexes: lookup yang sering dipakai
CREATE INDEX idx_emp_nik ON employees (nik);
CREATE INDEX idx_emp_site ON employees (site_name);
CREATE INDEX idx_emp_status ON employees (employment_status);
CREATE INDEX idx_emp_dept ON employees (department);
CREATE INDEX idx_emp_site_status ON employees (site_name, employment_status);
...
CREATE INDEX idx_mp_site_bulan_tahun ON man_power (jobsite, bulan, tahun);
CREATE INDEX idx_mp_bulan_tahun ON man_power (bulan, tahun);

-- absensi_sakit: filter per site, bulan, tahun
CREATE INDEX idx_abs_site ON absensi_sakit (jobsite);
CREATE INDEX idx_abs_bulan_tahun ON absensi_sakit (bulan, tahun);
CREATE INDEX idx_abs_site_bulan_tahun ON absensi_sakit (jobsite, bulan, tahun);
CREATE INDEX idx_abs_pak ON absensi_sakit (is_pak) WHERE is_pak = true;

-- kunjungan_berobat: filter per site, tanggal, rujuk
CREATE INDEX idx_kb_site ON kunjungan_berobat (jobsite);
CREATE INDEX idx_kb_tanggal ON kunjungan_berobat (tanggal);
CREATE INDEX idx_kb_bulan_tahun ON kunjungan_berobat (
  EXTRACT(MONTH FROM tanggal), EXTRACT(YEAR FROM tanggal)
);
CREATE INDEX idx_kb_rujuk ON kunjungan_berobat (rujuk_rs) WHERE rujuk_rs = true;
CREATE INDEX idx_kb_diagnosa ON kunjungan_berobat (diagnosa);
